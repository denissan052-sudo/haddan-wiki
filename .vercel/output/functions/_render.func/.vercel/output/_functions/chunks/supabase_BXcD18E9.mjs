import { c as createComponent, d as addAttribute, e as renderHead, f as renderSlot, a as renderTemplate, b as createAstro, m as maybeRenderHead } from './astro/server_Dbmzirj3.mjs';
import 'kleur/colors';
import 'clsx';
/* empty css                             */
...
Уже есть аккаунт? <button type="button" id="show-login" class="text-haddan-gold hover:underline">Войти</button> </p> </form> </div> <div id="auth-error" class="mt-4 p-3 bg-haddan-red/10 border border-haddan-red/30 rounded-lg text-haddan-red text-sm hidden"></div> </div> </div> `;
